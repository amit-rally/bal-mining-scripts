...
function getBalAndRatioFactor(tokens, weights) {
    let balFactorRatioFactorSum = bnum(0);
    ...
    for (let j = 0; j < n; j++) {
        if (!weights[j].eq(bnum(0))) {
            for (let k = j + 1; k < n; k++) {
                ...
                let pairBalFactorRatioFactor = balFactor
                    .times(bnum(4)) // stretches factor for equal weighted pairs to 1
                    .times(normalizedWeight1)
                    .times(normalizedWeight2)
                    .times(pairWeight);

                balFactorRatioFactorSum = balFactorRatioFactorSum.plus(
                    pairBalFactorRatioFactor
                );
    ...
    return balFactorRatioFactorSum.div(pairWeightSum);
}
